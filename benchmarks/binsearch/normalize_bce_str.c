...
static lua_Integer function_binsearch_titan(
    ...
    (void)x5;
    {
        ...
        lua_Unsigned x1size = x1->sizearray;
        ...
        for(;;) {
            ...
            if (!x10) break;
            {
                ...
                if (TITAN_UNLIKELY(x16 >= x1size)) {
                    titan_renormalize_array(L, x1, x16, 13);
                    x1size = x1->sizearray;
                    x1array = x1->array;
                }
                const TValue * x17 /* arrslot */ = &x1array[x16];
...
int luaopen_benchmarks_binsearch_normalize_bce_str(lua_State *L)
{
    lua_checkstack(L, 4);
