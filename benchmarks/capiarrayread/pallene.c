...
static lua_Number function_run_pallene(
    ...
    clock_gettime(CLOCK_MONOTONIC, &before);
        ...
        if (PALLENE_LIKELY(x5 < x1->sizearray)) {
            x6 = &x1->array[x5];
        }
        else {
            x6 = (TValue *)luaH_getint(x1, 1);
        }
        if (PALLENE_UNLIKELY(!ttisfloat(x6))) {
            pallene_runtime_array_type_error(L, 2, LUA_TNUMFLT, rawtt(x6));
        }
...
int luaopen_benchmarks_capiarrayread_pallene(lua_State *L)
{
    lua_checkstack(L, 4);
