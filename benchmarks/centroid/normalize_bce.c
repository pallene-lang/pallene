            ...
            (void) x13;
            {
                ...
                while ((x16 >= 0 ? x14 <= x15 : x14 >= x15)) {
                    ...
                    (void) x17;
                    {
                        ...
                        if (TITAN_UNLIKELY(!ttistable(x19))) {
                            titan_runtime_array_type_error(L, 13, LUA_TTABLE, rawtt(x19));
                        }
                        ...
                        if (TITAN_UNLIKELY(!ttisfloat(x23))) {
                            titan_runtime_array_type_error(L, 14, LUA_TNUMFLT, rawtt(x23));
                        }
                        lua_Number x24 = fltvalue(x23);
                        lua_Number x25 = x6 + x24;
                        x6 = x25;
                        const TValue * x27 /* arrslot */ = &x21->array[x26];
                        if (TITAN_UNLIKELY(!ttisfloat(x27))) {
                            titan_runtime_array_type_error(L, 15, LUA_TNUMFLT, rawtt(x27));
                        }
...
int luaopen_benchmarks_centroid_normalize_bce(lua_State *L)
{
    lua_checkstack(L, 4);
