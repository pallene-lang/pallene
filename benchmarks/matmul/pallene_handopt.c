...
static Table * function_matmul_pallene(
    ...
    (void)x5;
    {
        ...
        if (PALLENE_UNLIKELY(!ttistable(x15))) {
            pallene_runtime_array_type_error(L, 5, LUA_TTABLE, rawtt(x15));
        }
        ...
        /* BEGIN OPT - bounds for B[k]*/
        {
            lua_Unsigned bound = ((lua_Unsigned)x36) - 1;
        ...
        while ((x37 >= 0 ? x35 <= x36 : x35 >= x36)) {
            ...
            (void) x38;
            {
                ...
                if (PALLENE_UNLIKELY(!ttistable(x40))) {
                    pallene_runtime_array_type_error(L, 14, LUA_TTABLE, rawtt(x40));
                }
                ...
                /* BEGIN OPT - bounds for A[i], C[i]*/
                {
                    lua_Unsigned bound = ((lua_Unsigned)x44) - 1;
                    if (PALLENE_UNLIKELY(bound >= x1->sizearray)) {
                        pallene_renormalize_array(L, x1, bound, 19);
                    }
                    if (PALLENE_UNLIKELY(bound >= x9->sizearray)) {
                        pallene_renormalize_array(L, x9, bound, 19);
                    }
                }
                /*END OPT*/
                while ((x45 >= 0 ? x43 <= x44 : x43 >= x44)) {
                    ...
                    (void) x46;
                    {
                        ...
                        if (PALLENE_UNLIKELY(!ttistable(x48))) {
                            pallene_runtime_array_type_error(L, 16, LUA_TTABLE, rawtt(x48));
                        }
                        ...
                        if (PALLENE_UNLIKELY(!ttisfloat(x51))) {
                            pallene_runtime_array_type_error(L, 16, LUA_TNUMFLT, rawtt(x51));
                        }
                        ...
                        if (PALLENE_UNLIKELY(!ttistable(x55))) {
                            pallene_runtime_array_type_error(L, 17, LUA_TTABLE, rawtt(x55));
                        }
                        ...
                        /* BEGIN OPT - bounds for Ci[j], Bk[j] */
                        {
                            lua_Unsigned bound = ((lua_Unsigned)x59) - 1;
                            if (PALLENE_UNLIKELY(bound >= x57->sizearray)) {
                                pallene_renormalize_array(L, x57, bound, 19);
                            }
                            if (PALLENE_UNLIKELY(bound >= x42->sizearray)) {
                                pallene_renormalize_array(L, x42, bound, 19);
                            }
                        }
                        /*END OPT*/
                        while ((x60 >= 0 ? x58 <= x59 : x58 >= x59)) {
                            ...
                            (void) x61;
                            {
                                ...
                                if (PALLENE_UNLIKELY(!ttisfloat(x63))) {
                                    pallene_runtime_array_type_error(L, 19, LUA_TNUMFLT, rawtt(x63));
                                }
                                lua_Number x64 = fltvalue(x63);
                                const TValue * x66 /* arrslot */ = &x42->array[x62];
                                if (PALLENE_UNLIKELY(!ttisfloat(x66))) {
                                    pallene_runtime_array_type_error(L, 19, LUA_TNUMFLT, rawtt(x66));
                                }
...
static int function_matmul_lua(lua_State *L)
{
    ...
    if (PALLENE_UNLIKELY(!ttistable(x7))) {
        pallene_runtime_argument_type_error(L, "B", 1, LUA_TTABLE, x7);
    }
...
int luaopen_benchmarks_matmul_pallene_handopt(lua_State *L)
{
    lua_checkstack(L, 4);
