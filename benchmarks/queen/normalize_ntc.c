...
static int function_isplaceok_titan(
    ...
    (void)x6;
    {
        ...
        if (TITAN_UNLIKELY((x8 - 1) >= x1->sizearray)) {
            titan_renormalize_array(L, x1, x8 - 1, 4);
        }
...
static void function_printsolution_titan(
    ...
    (void)x5;
    {
        ...
        if (TITAN_UNLIKELY((x1 - 1) >= x2->sizearray)) {
            titan_renormalize_array(L, x2, x1 - 1, 18);
        }
                    ...
                    {
                        {
                            ...
                            if (x12) {
                                ...
                                if (TITAN_UNLIKELY(x13 >= x2->sizearray)) {
                                    titan_renormalize_array(L, x2, x13, 40);
                                }
                                TValue * x14 /* slot */ = &x2->array[x13];
                                setivalue(x14, x10);
                                lua_Integer x15 = intop(+, x3, 1);
                                StackValue* x16 = L->top;
                                sethvalue(L, s2v(x16), x2); x16++;
                                L->top = x16;
                                function_addqueen_titan(L, x1, x2, x15);
                                L->top -= 1;
                            }
                        }
                    }
...
int luaopen_benchmarks_queen_normalize_ntc(lua_State *L)
{
    lua_checkstack(L, 4);
