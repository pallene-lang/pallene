...
static Table * function_sieve_titan(
    ...
    (void)x4;
    {
        ...
        TValue *x7array_1 = x7->array + (x10 - 1);
        while ((x12 >= 0 ? x10 <= x11 : x10 >= x11)) {
            lua_Integer x13 /* n */ = x10;
            (void) x13;
            {
                setbvalue(x7array_1, 1);
            }
            x10 = intop(+, x10, x12);
            x7array_1 = x7array_1 + x12;
        }
        ...
        TValue *x7array_2 = x7->array;
        while ((x23 >= 0 ? x21 <= x22 : x21 >= x22)) {
            lua_Integer x24 /* n */ = x21;
            (void) x24;
            {
                {
                    const TValue * x26 /* arrslot */ = x7array_2;
                    if (TITAN_UNLIKELY(!ttisboolean(x26))) {
                        titan_runtime_array_type_error(L, 12, LUA_TBOOLEAN, rawtt(x26));
                    }
                    int x27 = bvalue(x26);
                    if (x27) {
                        ...
                        TValue *x7array_3 = x7->array + (x32 - 1);
                        while ((x34 >= 0 ? x32 <= x33 : x32 >= x33)) {
                            lua_Integer x35 /* m */ = x32;
                            (void) x35;
                            {
                                TValue * x37 /* slot */ = x7array_3;
                                setbvalue(x37, 0);
                            }
                            x32 = intop(+, x32, x34);
                            x7array_3 = x7array_3 + x34;
                        }
                    }
                }
            }
            x21 = intop(+, x21, x23);
            x7array_2 = x7array_2 + x23;
        }
        return x20;
    }
}
...
int luaopen_benchmarks_sieve_normalize_bce_str(lua_State *L)
{
    lua_checkstack(L, 4);
