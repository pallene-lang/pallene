            ...
            {
                {
                    ...
                    if (PALLENE_UNLIKELY(!ttisboolean(x26))) {
                        pallene_runtime_array_type_error(L, 12, LUA_TBOOLEAN, rawtt(x26));
                    }
                    int x27 = bvalue(x26);
                    if (x27) {
...
static int function_sieve_lua(lua_State *L)
{
    ...
    if (PALLENE_UNLIKELY(!ttisinteger(x6))) {
        pallene_runtime_argument_type_error(L, "N", 1, LUA_TNUMINT, x6);
    }
...
int luaopen_benchmarks_sieve_pallene_handopt(lua_State *L)
{
    lua_checkstack(L, 4);
