...
return {
    ...
    injectStream = function(t)
        make_stream = assert(t.make_stream_pln)
        stream_head = assert(t.stream_head_pln)
        stream_tail = assert(t.stream_tail_pln)
        stream_get  = assert(t.stream_get_pln)
    end,

    injectMain = function(t)
        count_from = assert(t.count_from_pln)
        sift       = assert(t.sift_pln)
        sieve      = assert(t.sieve_pln)
    end,
}
