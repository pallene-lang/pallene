...
function RecordCoder:declare_newindex(field_name)
    ...
    local out = util.render([[
        ...
        CHECK_TAG = check_tag(ctx, typ, 'v', false, "record field '%s'",
            C.string(field_name)),
...
function RecordCoder:create_metatable(ctx)
    ...
    table.insert(cstats, table_set_new_field(mt.name, "__metatable",
            types.T.Boolean(), C.boolean(false), ctx))
...
local function generate_lua_entry_point(tl_node, literals)
    ...
    local check_nargs = util.render([[
        ...
        NAME = C.string(tl_node.name),
        EXPECTED = C.integer(#tl_node.params),
    })
        ...
        if param._type._tag ~= "types.T.Value" then
            ...
            table.insert(check_types, util.render([[
                ...
                CHECK_TAG = check_tag(ctx, param._type, slot.name,
                    param.loc, "argument %s", C.string(param.name)),
            }))
        end
    end
    ...
    local out = util.render([[
        ...
        NRET = C.integer(#tl_node._type.ret_types),
    })
    return out
end
...
local function generate_luaopen_exports_table(prog_ast, ctx)
    ...
    table.insert(parts, util.render([[
        ...
        N = C.integer(n_exported_functions),
    }))
    for _, tl_node in ipairs(prog_ast) do
        if tl_node._tag == "ast.Toplevel.Func" and not tl_node.is_local then
            table.insert(parts,
                util.render([[
                    ...
                    NAME = C.string(ast.toplevel_name(tl_node)),
...
generate_exp = function(exp, ctx)
    ...
    elseif tag == "ast.Exp.Bool" then
        return "", C.boolean(exp.value)

    elseif tag == "ast.Exp.Integer" then
        return "", C.integer(exp.value)

    elseif tag == "ast.Exp.Float" then
        return "", C.float(exp.value)
    ...
    elseif tag == "ast.Exp.Cast" then
        ...
        elseif dst_typ._tag == "types.T.Value" then
            ...
            local cstats = util.render([[
                ...
                OUT_DECL = c_declaration(out),
                EXP_CSTATS = exp_cstats,
                -- Technically this is not a slot in the Lua stack,
                -- but it also belongs to no one special...
                SET_SLOT = set_stack_slot(src_typ, "&"..out.name, exp_cvalue)
            })
            return cstats, out.name
