...
describe("pallenec", function()
    ...
    it("Can compile pallene files", function()
        ...
        local ok, err, out, _ = util.outputs_of_execute("./vm/src/lua __test__script__.lua")
        assert(ok, err)
        assert.equals("17\n", out)
    end)
    ...
    it("Can compile with --output flag", function()
        ...
        local ok, err, out, _ = util.outputs_of_execute("./vm/src/lua __test__script__flag__.lua")
        assert(ok, err)
        assert.equals("17\n", out)
    end)
